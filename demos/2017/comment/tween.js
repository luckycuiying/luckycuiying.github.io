~ function(){
    ...
    // move: 实现多方向的运动动画：
    // curEle:当前要操作的运动元素，
    // target: 当前动画的目标位置(存储的是每一个方向的目标位置{left：嘻嘻嘻，top:xxx，...})
    // duration: 当前动画的总时间
    // effect: 支持一下情况：
    //  1》 如果传进来的是数字的话：
    //  0: Linear ;
    //  1: Circ.easeInOut(圆形曲线的缓动);
    //  2:Elastic.easeOut 指数衰减的正弦曲线缓动
    //  3: Back.easeOut 超过范围的三次方缓动
    //  4:Bounce.easeOut 指数衰减的反弹缓动
    //  5:Expo.easeIn  指数曲线的缓动
    //  2》 如果传进来的是一个数组：mover(curEle,target,duration,["Elastic","easeOut"]) -->myEffect.Elastic.easeOut
    //  3》如果不传递的话，就默认使用my.Effect.Linear
    function move(curEle,target,duration,effect,callBack){
        ...
        curEle.timer = window.setInterval(function(){
            ...
            // 未到达目标：分别的获取每一个方向的当前位置，给当前元素设置样式即可
            for(var key in target){
                if(target.hasOwnProperty(key)){
